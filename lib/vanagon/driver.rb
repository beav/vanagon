...
class Vanagon
  class Driver
    ...
    def run # rubocop:disable Metrics/AbcSize
      ...
      @engine.dispatch("(cd #{@engine.remote_workdir}; #{@platform.make})") unless @platform.srpm_only
    ...
    def prepare(workdir = nil) # rubocop:disable Metrics/AbcSize
      ...
      @engine.ship_workdir(@workdir)
      @engine.dispatch("(cd #{@engine.remote_workdir}; #{@platform.make} #{@project.name}-project)") unless @platform.srpm_only
    rescue => e
      puts e
      puts e.backtrace.join("\n")
      raise e
